...
df1 <- merge(W[idx,] %>% t(), meta.dat[,'grp', drop =F], by = 0) %>% column_to_rownames('Row.names')%>% melt() %>% 
  left_join(tmp[,'log2FoldChange',drop =F] %>% rownames_to_column('variable')) %>% 
  mutate(variable = paste0(variable, '(LFC=',round(log2FoldChange,3),')'))
head(df1)
p1 <- ggplot(df1,aes(x=grp,y = value, color=grp)) + 
...
p1
df2 <- merge(prop[idx,] %>% t(), meta.dat[,'grp', drop =F], by = 0) %>% column_to_rownames('Row.names')%>% melt() %>%
  left_join(tmp[,'log2FoldChange',drop =F] %>% rownames_to_column('variable')) %>% 
  mutate(variable = paste0(variable, '(LFC=',round(log2FoldChange,3),')'))

p2 <- ggplot(df2, aes(x=grp,y = sqrt(value), color=grp)) + 
  geom_boxplot(outlier.shape = NA) + geom_jitter(width = 0.2, size=0.8, alpha=0.5) + 
  facet_wrap(~variable, scale='free', nrow= 2) + 
  scale_color_brewer(palette = 'Dark2') + 
...
ggsave(file=paste0(rd,'Figure3.pdf'), width = 12, height = 12)
